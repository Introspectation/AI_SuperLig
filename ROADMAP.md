...
| 7. Small-data ML challengers | Complete | Regularized, low-capacity challengers using legal pre-match features only. | Out-of-sample gain and calibration justify complexity. |
| 8. Candidate freeze | Next | Select model and calibration rule using development evidence. | Code, hyperparameters, and decision rule frozen before holdout. |
...
at the grid maximum (`reports/PROMOTED_PRIOR_REPORT.md`). Stage 7's penalized
logistic challenger does not beat Stage 6 and is reported as characterized. A
plain logistic ablation is marginally lower on point estimates, but its interval
includes zero (`reports/ML_CHALLENGER_REPORT.md`). Stage 8 is next and needs a
new user go-ahead.
